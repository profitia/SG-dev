/**
 * app/api/supplier/intelligence/route.ts
...
export const GET = withCognitionAuth(async (auth, request: NextRequest) => {
  ...
  const result = await getCognitionService().getSupplierIntelligence(auth.orgId, auth.requestId, parsed.data.supplierRef)
