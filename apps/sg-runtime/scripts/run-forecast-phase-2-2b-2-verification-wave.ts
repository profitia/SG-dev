...
async function main() {
  ...
  if (requests.length !== 2) {
    throw new Error('Phase 2.2B-2 Verification wave must contain exactly 2 requests.')
  }
  if (requests.some((request) =>
    request.operation !== 'VERIFICATION'
    || request.scenarioId !== 'B2-P08-VERIFICATION-2'
    ...
    throw new Error('Phase 2.2B-2 adapter permits only the frozen Generic Period P08 Verification identity.')
  }
  if (new Set(requests.map(({ stressRunId }) => stressRunId)).size !== 1) {
    throw new Error('Phase 2.2B-2 Verification wave requires one stressRunId.')
  }
  if (new Set(requests.map(({ modelId }) => modelId)).size !== 1) {
    throw new Error('Phase 2.2B-2 Verification wave requires one exact model identity.')
  }

  const results = await runForecastStressOperationWave(requests)
  process.stdout.write(`[FORECAST_PHASE_2_2B_2_RESULT] ${JSON.stringify(results)}\n`)
}
