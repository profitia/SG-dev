...
Successor: `Canon/v1.1-developer-lifecycle-specification.md`

Current Execution Amendment: The current executable lifecycle is governed by `Canon/v2.0-mandatory-agent-execution-canon.md`. Historical stage descriptions in this v1 specification remain architectural background; where they describe a single-phase or VSC-specific execution flow, v2 and the current implementation prevail.
...
## 14. PMOS Final Handoff Timing Invariant

PMOS final task continuity MUST be based on terminal closeout evidence,
not on bootstrap input, pending narrative, or pre-save handoff content.

A pre-save HANDOFF may exist only as a draft or pending execution artifact.
It MUST NOT be treated as the final persisted state of a task.

A task may be reported as completed only when the persisted HANDOFF reflects
the final observed execution state, including:

- PMOS Save status;
- closeout state;
- ConversationArtifact persistence;
- HANDOFF artifact persistence;
- publication status where applicable;
- Publication ACK where applicable;
- pending artifact slot final state;
- runtime verification status;
- known downstream readiness signals.

If PMOS Save succeeds, closeout reaches `CLOSEOUT_COMPLETE`, publication is
delivered, Publication ACK is present, and the pending artifact slot is clear,
then the persisted HANDOFF MUST NOT continue to report a pending or pre-closeout
state.

Publication ACK MUST NOT be treated as MEMOROS Knowledge Ready.

MEMOROS Knowledge Ready remains a downstream consumer-readiness state and MUST
be reported separately from PMOS completion and publication success.
