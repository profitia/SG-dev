...
const OUTPUT_PATH = path.join(FORECAST_ROOT, 'validation', 'phase-2-2b-4r', 'functional-regression.json')
const PROHIBITED = /phase-2-1b-baseline|phase-2-2b-4r-controlled-stress|--execute|stress workload/i
...
function executeCheck(check) {
  ...
  assert.equal(PROHIBITED.test(commandText), false, `Prohibited B4R regression command: ${commandText}`)
...
async function checks() {
  ...
  const forecastTests = path.join(FORECAST_ROOT, 'tests')
  const runtimeTests = await testFiles(path.join(runtimeRoot, 'tests'), '.test.ts')
  const toolingTests = await testFiles(PERFORMANCE_ROOT, '.test.mjs')
  return [
    {
      id: 'forecast-core',
      category: 'FUNCTIONAL_TEST',
      command: path.join(FORECAST_ROOT, '.venv', 'bin', 'python'),
      args: ['-m', 'unittest', 'discover', '-s', forecastTests],
      cwd: FORECAST_ROOT,
    },
    {
      id: 'sg-runtime',
      ...
      args: ['--import', 'tsx', '--test', ...runtimeTests],
      cwd: runtimeRoot,
    },
    {
    ...
    },
    {
      ...
      category: 'FUNCTIONAL_TEST',
      command: process.execPath,
      args: ['--test', ...toolingTests],
      cwd: REPOSITORY_ROOT,
    },
    {
      id: 'sg-runtime-typecheck',
      category: 'TYPECHECK',
      command: 'npm',
      args: ['run', 'typecheck'],
      cwd: runtimeRoot,
    ...
    {
      id: 'dashboard-preview-typecheck',
      category: 'TYPECHECK',
      command: 'npm',
      args: ['run', 'typecheck'],
      cwd: dashboardRoot,
    ...
    ...[
      ...
      command: process.execPath,
      args: [path.join(PERFORMANCE_ROOT, file)],
      cwd: REPOSITORY_ROOT,
    })),
  ]
}
...
async function run() {
  ...
  const evidence = {
    task: 'FORECAST_PHASE_2_2B_4R_FUNCTIONAL_REGRESSION',
    generatedAt,
  ...
  assert.equal(evidence.status, 'PASS', `Functional regression failed: ${results.filter(({ status }) => status === 'FAIL').map(({ id }) => id).join(', ')}`)
  process.stdout.write(`${JSON.stringify({
    phase22b4rFunctionalRegression: evidence.status,
    checksPassed: evidence.checksPassed,
    checksExpected: evidence.checksExpected,
    stressExecutionObserved: evidence.stressExecutionObserved,
  }, null, 2)}\n`)
}
...
if (process.argv[2] === '--run') {
  ...
  process.stderr.write('Usage: phase-2-2b-4r-functional-regression.mjs --run\n')
  process.exitCode = 1
}